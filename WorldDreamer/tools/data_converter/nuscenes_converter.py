...
def create_nuscenes_infos(
    ...
    train_nusc_infos, train_scene_tokens, val_nusc_infos, val_scene_tokens = (
...
def _sample_data_transform(
    nusc, sample, max_sweeps, ego_pose_token=None, cam_tokens=None, keyframe=True
):
    ...
    location = nusc.get("log", nusc.get("scene", sample["scene_token"])["log_token"])[
        "location"
    ]
    description = nusc.get("scene", sample["scene_token"])["description"]
    timeofday = nusc.get("log", nusc.get("scene", sample["scene_token"])["log_token"])[
        "logfile"
    ][5:]
    ...
    info["visibility"] = np.array(
        [a["visibility_token"] for a in annotations], dtype=np.uint8
    )
    info["valid_flag"] = valid_flag
    info["obj_ids"] = np.array([a["instance_token"] for a in annotations])
    return info
...
def _fill_trainval_infos(
    nusc, train_scenes, val_scenes, test=False, max_sweeps=10, with_cam_sweeps=True
):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if (
            (len(infos) == frames_t or len(infos) == 1)
            or not use_break
            or not with_cam_sweeps
        ):
