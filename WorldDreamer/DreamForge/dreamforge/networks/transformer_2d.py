...
class MultiviewTransformer2DModel(ModelMixin, ConfigMixin):
    """
    ...
    def forward(
        ...
        if self.is_input_continuous:
            ...
            embedding = self.scene_proj_in(embedding).permute(0, 2, 3, 1).reshape(batch, height * width, -1)
            ##################
...
class TemporalMultiviewTransformer2DModel(ModelMixin, ConfigMixin):
    """
    Transformer model for image-like data. Takes either discrete (classes of vector embeddings) or continuous (actual
    embeddings) inputs.
