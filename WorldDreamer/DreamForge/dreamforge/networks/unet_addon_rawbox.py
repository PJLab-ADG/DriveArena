...
class Upsampler(nn.Sequential):
    def __init__(self, in_dim, out_dim, scale=4, pixel_shuffle=False, bn=False, act=False, bias=True):
        ...
        if pixel_shuffle:
            if (scale & (scale - 1)) == 0:    # Is scale = 2^n?
                for _ in range(int(math.log(scale, 2))):
                    m.append(nn.Conv2d(in_dim, 4*out_dim, kernel_size=1, bias=bias))
                    if bn:
                        m.append(nn.BatchNorm2d(out_dim))
                    if act == 'relu':
                        m.append(nn.ReLU(True))
                    elif act == 'prelu':
                        m.append(nn.PReLU(out_dim))
                    m.append(nn.PixelShuffle(2))
                    in_dim = out_dim
        else:
            m.append(nn.Conv2d(in_dim, out_dim, kernel_size=1, bias=bias))
            if bn:
                m.append(nn.BatchNorm2d(out_dim))
            if act == 'relu':
                m.append(nn.ReLU(True))
                ...
                m.append(nn.PReLU(out_dim))
            m.append(nn.Upsample(scale_factor=scale, mode='nearest'))

        m.append(zero_module(nn.Conv2d(out_dim, out_dim, kernel_size=3, padding=1, bias=bias)))
...
class BEVControlNetModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        with_upsampler: bool = False,
        upsampler_out_channels: Tuple[int] = (256, 512, 512, 1024),
    ):
        super().__init__()
        logging.debug(
        ...
        ##### upsampler
        self.with_upsampler = with_upsampler
        if self.with_upsampler:
            self.controlnet_down_upsamplers = nn.ModuleList([])
            self.controlnet_down_upsamplers.append(Upsampler(output_channel, upsampler_out_channels[0]))
        #####
        ...
        for i, down_block_type in enumerate(down_block_types):
            ...
            for _ in range(layers_per_block):
                ...
                ##### upsampler
                if self.with_upsampler:
                    self.controlnet_down_upsamplers.append(Upsampler(output_channel, upsampler_out_channels[i]))
                #####
        ...
        ##### upsampler
        if self.with_upsampler:
            self.controlnet_mid_upsampler = Upsampler(mid_block_channel, upsampler_out_channels[-1])
        #####
    ...
    def _only_condition_encoding(self):
    ...
    def trainable_module(self) -> Dict[str, nn.Module]:
        assert self.with_upsampler
        self._new_module = {}
        for i, mod in enumerate(self.controlnet_down_upsamplers):
            self._new_module[f'controlnet_down_upsamplers.{i}'] = mod
        self._new_module['controlnet_mid_upsampler'] = self.controlnet_mid_upsampler
    ...
    def forward(
        ...
        controlnet_cond: Union[torch.FloatTensor, List[torch.FloatTensor]],
        ...
        adapting_upsampler: bool = False,
        **kwargs,
    ) -> Union[BEVControlNetOutput, Tuple]:
        # check channel order
        channel_order = self.config.controlnet_conditioning_channel_order

        if isinstance(controlnet_cond, list):
            controlnet_cond, layout_canvas = controlnet_cond[0], controlnet_cond[1]
        else:
            layout_canvas = None
        ...
        if self.with_upsampler and adapting_upsampler:
            controlnet_down_block_res_samples = ()

            for down_block_res_sample, controlnet_block, controlnet_upsampler in zip(
                down_block_res_samples, self.controlnet_down_blocks, self.controlnet_down_upsamplers
            ):  
                down_block_res_sample = controlnet_upsampler(controlnet_block(down_block_res_sample))
                controlnet_down_block_res_samples += (down_block_res_sample,)

            down_block_res_samples = controlnet_down_block_res_samples

            mid_block_res_sample = self.controlnet_mid_upsampler(self.controlnet_mid_block(sample))
        else:
            controlnet_down_block_res_samples = ()
