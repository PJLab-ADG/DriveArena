...
def collate_fn_single(
    ...
    pixel_values = pixel_values.to(
        memory_format=torch.contiguous_format).float()
    if 'hr_img' in examples[0]:
        hr_pixel_values = torch.stack([example["hr_img"].data for example in examples])
        hr_pixel_values = hr_pixel_values.to(
            memory_format=torch.contiguous_format).float()
    ...
    elif "gt_masks_bev" in examples[0]:
        keys = ["gt_masks_bev"]
    else:
        keys = []
    # fmt: off
    ...
    if 'hr_img' in examples[0]:
        ret_dict['hr_pixel_values'] = hr_pixel_values
    ...
    if 'layout_canvas' in examples[0]:
