...
class ScoreCalculator:
    ...
    def get_vehicle_corners(self, x, y, yaw, length, width):
        ...
        front_left = np.array(
            [x + c * length / 2 - s * width / 2, y + s * length / 2 + c * width / 2]
        )
    ...
    def plot_trajectory_on_drivable_mask(
        self, drivable_mask, transformed_traj, vehicle_width, vehicle_length
    ):
        """
        ...
        for x, y, yaw in transformed_traj:
            corners = self.get_vehicle_corners(x, y, yaw, vehicle_length, vehicle_width)
    ...
    def _calculate_drivable_area_compliance(
        self, drivable_mask, transformed_traj, vehicle_width, vehicle_length
    ):
        """
        ...
        for traj_ti, (x, y, yaw) in enumerate(transformed_traj):
            ...
            corners_px = np.array([meter_to_pixel(cx, cy) for cx, cy in corners])
            ...
            drivable_ratio = (
                drivable_vehicle_pixels / total_vehicle_pixels
                if total_vehicle_pixels > 0
                else 1.0
            )
    ...
    def _calculate_progress(self, planned_traj, ref_taj):
        ...
        def project_curve_onto_curve(curve_a, curve_b):
            ...
            for point in curve_b.coords:
                projected_point = curve_a.interpolate(curve_a.project(Point(point)))
        ...
        print("planned traj: ", planned_traj)
        print("reference traj: ", ref_taj)

        print(
            f"plan_curve_length: {plan_curve_length}, ref_curve_length: {ref_curve_length}, project plan to ref_length: {projected_length}"
        )
    ...
    def _calculate_is_comfortable(self, traj, timestep):
        ...
        checks = [
            np.all(np.abs(kinematics["lat_accel"]) <= boundaries["max_abs_lat_accel"]),
            np.all(kinematics["lon_accel"] <= boundaries["max_abs_lat_accel"]),
            np.all(kinematics["lon_accel"] >= boundaries["min_lon_accel"]),
            np.all(np.abs(kinematics["lon_jerk"]) <= boundaries["max_abs_lon_jerk"]),
            np.all(np.abs(kinematics["yaw_accel"]) <= boundaries["max_abs_yaw_accel"]),
            np.all(np.abs(kinematics["yaw_rate"]) <= boundaries["max_abs_yaw_rate"]),
        ]
    ...
    def calculate(
        ...
        print(f"current exp has {len(self.data['infos'])} frames")
        if len(self.data["infos"]) == 0:
            return None
        # todo: time_step need modify
        score_list = {}
        ep_man = 0.00
        for i in range(0, len(self.data["infos"]), 1):
            frame = self.data["infos"][i]
            # if frame["is_key_frame"] == False:
            #     continue

            print(f"frame {i} / {len(self.data['infos'])}")
            timestamp = frame["timestamp"]
            planned_last_timestamp = (
                timestamp
                + len(frame["planned_traj"]["traj"]) * frame["planned_traj"]["timestep"]
            )
            ...
            planned_traj = np.concatenate(
                ([np.array([ego_x, ego_y, ego_yaw])], frame["planned_traj"]["traj"]),
                axis=0,
            )
            ...
            while current_timestamp <= planned_last_timestamp + 1e-5:
                if (
                    abs(
                        current_timestamp
                        - self.data["frames"][current_frame_idx]["time_stamp"]
                    )
                    < 1e-5
                ):
                    obs_list = []
                    for idx, obj in enumerate(
                        ...
                        # obs_list.append(obj)
                        if (
                            self.data["frames"][current_frame_idx]["obj_names"][idx]
                            == "car"
                        ):
                            obs_list.append(obj)
            ...
            score_pdms = (
                ...
                * score_dac
                * (
                    score_weight["ttc"] * score_ttc
                    + score_weight["c"] * score_c
                    + score_weight["ep"] * score_ep
                )
        ...
        averages = {metric: total / num_entries for metric, total in totals.items()}
        print("num_entries: ", num_entries)
        print("average ep: ", ep_man / num_entries)
        # write to csv
        ...
        if self.data["type"] == "closed-loop":
            ...
            route_completion = self.data["drive_length"] / self.data["route_length"]
...
def main(data):
    ...
    def multi_threaded_process(data, max_workers=None):
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(process_exp_data, data)]
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        file_name = "/home/PJLAB/weitiantian/repo/DriveArena/results/09-23-110851/drive_arena.pkl"
