...
def limsim2diffusion(vehicles, data_template, vectorized_map:VectorizedLocalMap, map_name, agent_command =2, last_pose = torch.eye(4), drivable_mask=np.ones((200,200), dtype=np.uint8), accel=[0,0,9.81], rotation_rate=[0,0,0], vel=[5,0,0], gen_location = "singapore-onenorth", gen_prompts ="daytime, cloudy, downtown, gray buildings, white cars"):
    VEH_LENGTH = 4.7
    VEH_WIDTH = 1.6
    VEH_HEIGHT= 1.4

    ego_vehicle = vehicles['egoCar']
    ego_x, ego_y, ego_yaw = ego_vehicle['xQ'][-1], ego_vehicle['yQ'][-1], ego_vehicle['yawQ'][-1]
    ...
    for sur_veh in vehicles['carInAoI']:
        sur_x, sur_y, sur_yaw = sur_veh['xQ'][-1], sur_veh['yQ'][-1], sur_veh['yawQ'][-1]
        tran_x, tran_y, tran_yaw = transform((sur_x, sur_y, sur_yaw), (ego_x, ego_y, ego_yaw))
    ...
    # ------------ HDMap ------------ #
    anns_results = vectorized_map.gen_vectorized_samples(map_name, [ego_x, ego_y], np.deg2rad(ego_yaw_deg-90))
    ...
    # ---------------ref pose------------------#
    send_data['relative_pose'] = torch.matmul(torch.inverse(send_data['metas']['ego_pos']), last_pose)
...
def transform_to_ego_frame(curr_state: State, target_state: State):
    ...
    translated = np.array([target_state.x - curr_state.x,
                          target_state.y - curr_state.y])
...
def interpolate_traj(ego_vehicle, path_points, Ti_path=0.5) -> Trajectory:
    ...
    global_points = [
        (
            ego_x + px * math.cos(ego_yaw-math.pi/2) -
            py * math.sin(ego_yaw-math.pi/2),
            ego_y + px * math.sin(ego_yaw-math.pi/2) +
    ...
    for i in range(1, len(states)):
        ...
        for _ in range(int((curr_state.t - prev_state.t) / Ti_traj)):
            ...
            trajectory.states.append(State(
                ...
                yaw=prev_state.yaw + ratio *
                normalize_angle(curr_state.yaw - prev_state.yaw),
