    ...
    def send_request_diffusion(self, diffusion_data: Dict) -> Optional[np.ndarray]:
        serialized_data = {
            k: (
                ...
                if isinstance(v, torch.Tensor)
                    ...
                    {
                        k2: v2.numpy().tolist() if isinstance(v2, torch.Tensor) else v2
        ...
        try:
            ...
            if (
                ...
                cv2.imwrite(
                    f"{self.img_save_path}diffusion_{str(int(self.timestamp*2)).zfill(3)}.jpg",
    ...
    def process_frame(self):
        ...
        if self.model.timeStep % 5 == 0:
            ...
            resized_images = [
                ...
                for img in [front_left_image, front_image, front_right_image]
            ]
            ...
            if len(limsim_trajectories[self.EGO_ID].states) < 10:
                ...
                yaw_rate = (
                    limsim_trajectories[self.EGO_ID].states[9].yaw
                    - limsim_trajectories[self.EGO_ID].states[0].yaw
                )
            vx_1, vx_2 = (
                path_points[2][0] - path_points[0][0],
                path_points[3][0] - path_points[1][0],
            )
            vy_1, vy_2 = (
                path_points[2][1] - path_points[0][1],
                path_points[3][1] - path_points[1][1],
            )
            ...
            self.scorer.record_frame(
                drivable_mask,
                is_planning_frame=True,
                planned_traj=ego_traj,
                ref_traj=limsim_trajectories[self.EGO_ID],
            )

            limsim_trajectories = {}
            if self.USE_AGENT_PATH and self.timestamp > 3.5:
